...
class LightOpenID
{
    ...
    function __construct($host, $proxy = null)
    {
        ...
        if(!function_exists('curl_init') && !in_array('https', stream_get_wrappers())) {
            throw validation::safeOutputToScreenException('You must have either https wrappers or curl enabled.');
        }
    }
        ...
        if (!empty($proxy)) {
            ...
            if ($proxy && !empty($proxy['host'])) {
                ...
                if (array_key_exists('port', $proxy)) {
                    ...
                    if ($proxy['port'] <= 0) {
                        throw validation::safeOutputToScreenException('The specified proxy port number is invalid.');
                    }
                }
    ...
    protected function request_curl($url, $method='GET', $params=array(), $update_claimed_id)
    {
        ...
        if (curl_errno($curl)) {
            throw validation::safeOutputToScreenException(curl_error($curl), curl_errno($curl));
        }
    ...
    protected function request_streams($url, $method='GET', $params=array(), $update_claimed_id)
    {
        if(!$this->hostExists($url)) {
            throw validation::safeOutputToScreenException("Could not connect to $url.", 404);
        }
    ...
    {
        if (!$url) throw validation::safeOutputToScreenException('No identity supplied.');
        ...
        for ($i = 0; $i < 5; $i ++) {
            ...
            throw validation::safeOutputToScreenException("No OpenID Server found at $url", 404);
        }
        throw validation::safeOutputToScreenException('Endless redirection!', 500);
    }
